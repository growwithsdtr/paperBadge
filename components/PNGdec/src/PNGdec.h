...
#ifndef __PNGDEC__
#define __PNGDEC__
// Local patch for PaperS3BookReader: add ESP_PLATFORM to the
// non-Arduino branch so PNGdec compiles under ESP-IDF (mirrors what
// upstream JPEGDEC does in its newer revisions). Without this the
// build looks for <Arduino.h> which doesn't exist with framework=espidf.
